...
generatedAt: 2026-06-14T15:26:06.964Z
model: gpt-4o-mini
...
## Purpose
This OpenAPI tag covers the management of link map entries within the LinkShift Management API for API-key clients.

## Endpoints
- **`GET /api/v1/link-map-entries`** (`listLinkMapEntries`)
  - Lists link map entries for a specified `linkMapId`. Supports pagination with a default `limit` of 20 and a maximum of 100. Optional `search` parameter allows scanning of keys and destinations (max 1024 chars).
  - **Response fields:** `data`, `hasMore`, `moreStartingAfterId`.

- **`POST /api/v1/link-map-entries`** (`createLinkMapEntry`)
  - Creates a new link map entry by adding a key-to-destination mapping. Keys are path/query fragments, not full URLs. Returns a 400 error for invalid key characters or destination URLs, and a 404 error for unknown `linkMapId`.
...
- **`POST /api/v1/link-map-entries/import`** (`importLinkMapEntries`)
  - Imports up to 500 link map entries in a single request (create-only). Returns HTTP 200 even if some rows fail, with details on `importedCount`, `failedCount`, and `errors`.
...
- **`GET /api/v1/link-map-entries/{id}`** (`getLinkMapEntry`)
  - Fetches a single link map entry by its ID. Returns a 404 error if the entry is soft-deleted or not within the organization.
  - **Response fields:** `id`, `linkMapId`, `key`, `keyNormalized`, `destination`, `createdAt`, `updatedAt`, `deletedAt`.

- **`PUT /api/v1/link-map-entries/{id}`** (`updateLinkMapEntry`)
  - Updates an existing link map entry by changing its `key` and/or `destination`. Requires at least one field to be sent. Returns a 400 error for validation failures (e.g., duplicate key) and a 404 error for out-of-scope IDs.
  - **Request body fields:** `key`, `destination`.
  - **Response fields:** `id`, `linkMapId`, `key`, `keyNormalized`, `destination`, `createdAt`, `updatedAt`, `deletedAt`.

- **`DELETE /api/v1/link-map-entries/{id}`** (`deleteLinkMapEntry`)
  - Soft-deletes a single link map entry identified by its ID. For bulk deletions, use the bulk-delete endpoint instead.
...
- **Error Codes:**
  ...
  - `404`: Resource not found or out of organization scope.
- **Rate Limits:** Implement backoff with jitter; do not hard-code limits.
...
- **`GET /api/v1/organization/usage`** (Organization) - To check API usage and limits.
