...
QR code generation and redirect tracing are served by a **separate public tools service**, not the LinkShift Management API.

## Before you start

- No API key or organization account is required for these endpoints.
- They are unrelated to redirect rules, link maps, or `X-API-Key` Management API calls.
- Signed-in users can use the same tools in the dashboard — see [Tools in the dashboard](./dashboard/tools-in-dashboard.md).
...
The redirect tester UI on `/redirect-tester` or `/tools/redirect-tester` may follow a full chain **client-side** by calling trace repeatedly. Each API call still returns only one hop.
