...
export class QrCodeService {
  async generate(rawUrl: string, format: QrCodeFormat, size: number): Promise<GeneratedQrCodeAsset> {
    ...
    if (format === 'svg') {
      return {
        payload: this.renderSvg(url, size),
  ...
  private renderEps(value: string, preferredSize: number): string {
    ...
    const { moduleSize, quietZone, totalModules, canvasSize } = this.resolveCanvasSize(
      qr.modules.size,
      preferredSize,
    );
  ...
  private renderSvg(value: string, preferredSize: number): string {
    ...
    const { moduleSize, quietZone, canvasSize } = this.resolveCanvasSize(qr.modules.size, preferredSize);
    const rects: string[] = [];
